...
public class EligibleCoupleService {
    ...
    public void fpChange(FormSubmission submission) {
        ...
        allTimelineEvents.add(forChangeOfFPMethod(submission.entityId(), submission.getFieldValue(AllConstants.ECRegistrationFields.CURRENT_FP_METHOD),
                submission.getFieldValue(AllConstants.NEW_FP_METHOD_FIELD_NAME), fpMethodChangeDate));
        allEligibleCouples.mergeDetails(submission.entityId(), EasyMap.mapOf(AllConstants.ECRegistrationFields.CURRENT_FP_METHOD, submission.getFieldValue(AllConstants.NEW_FP_METHOD_FIELD_NAME)));
    }
