...
public class FormDataRepository extends DrishtiRepository {
    ...
    public String saveFormSubmission(String paramsJSON, String data, String formDataDefinitionVersion) {
        ...
        return params.get(AllConstants.INSTANCE_ID_PARAM);
    }
    ...
    public List<FormSubmission> getPendingFormSubmissions() {
        SQLiteDatabase database = masterRepository.getReadableDatabase();
        Cursor cursor = database.query(FORM_SUBMISSION_TABLE_NAME, FORM_SUBMISSION_TABLE_COLUMNS, SYNC_STATUS_COLUMN + " = ?", new String[]{SyncStatus.PENDING.value()}, null, null, null);
    ...
    public long getPendingFormSubmissionsCount() {
        return longForQuery(masterRepository.getReadableDatabase(), "SELECT COUNT(1) FROM " + FORM_SUBMISSION_TABLE_NAME
                ...
                new String[]{SyncStatus.PENDING.value()});
    }

    public void markFormSubmissionsAsSynced(List<FormSubmission> formSubmissions) {
        SQLiteDatabase database = masterRepository.getWritableDatabase();
        for (FormSubmission submission : formSubmissions) {
            FormSubmission updatedSubmission = new FormSubmission(submission.instanceId(), submission.entityId(), submission.formName(), submission.instance(), submission.version(), SyncStatus.SYNCED, "1");
    ...
    public String saveEntity(String entityType, String fields) {
        ...
        String entityId = updatedFieldsMap.get(AllConstants.ENTITY_ID_FIELD_NAME);
    ...
    private ContentValues createValuesForFormSubmission(Map<String, String> params, String data, String formDataDefinitionVersion) {
        ContentValues values = new ContentValues();
        values.put(INSTANCE_ID_COLUMN, params.get(AllConstants.INSTANCE_ID_PARAM));
        values.put(ENTITY_ID_COLUMN, params.get(AllConstants.ENTITY_ID_PARAM));
        ...
        String syncStatus = SyncStatus.PENDING.value();
        if (params.containsKey(AllConstants.SYNC_STATUS)) {
            syncStatus = params.get(AllConstants.SYNC_STATUS);
        }
