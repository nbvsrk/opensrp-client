...
public class ChildDetailController {
    ...
    private List<org.ei.opensrp.view.contract.TimelineEvent> getEvents() {
        List<TimelineEvent> events = allTimelineEvents.forCase(caseId);
        List<org.ei.opensrp.view.contract.TimelineEvent> timelineEvents = new ArrayList<org.ei.opensrp.view.contract.TimelineEvent>();

        Collections.sort(events, new TimelineEventComparator());

        for (TimelineEvent event : events) {
            DateTimeFormatter dateTimeFormatter = DateTimeFormat.forPattern("dd-MM-YYYY");
            timelineEvents.add(new org.ei.opensrp.view.contract.TimelineEvent(event.type(), event.title(), new String[]{event.detail1(), event.detail2()}, event.referenceDate().toString(dateTimeFormatter)));
        }
