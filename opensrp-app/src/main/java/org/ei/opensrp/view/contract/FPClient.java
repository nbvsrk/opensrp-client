...
public class FPClient implements FPSmartRegisterClient {
    ...
    public FPClient setRefillFollowUp() {
        ...
        if (fpReferralFollowUpAlert != null) {
            this.withRefillFollowUps(new RefillFollowUps(REFERRAL_FOLLOW_UP, fpReferralFollowUpAlert, Context.getInstance().getStringResource(org.ei.opensrp.R.string.str_referral)));
        } else if (fpFollowUpAlert != null) {
            this.withRefillFollowUps(new RefillFollowUps(FP_FOLLOW_UP, fpFollowUpAlert, Context.getInstance().getStringResource(org.ei.opensrp.R.string.str_follow_up)));
        } else {
    ...
    private RefillFollowUps getOtherFPMethod(List<AlertDTO> alerts) {
        for (AlertDTO alert : alerts) {
            if (isOtherFPMethodAlert(alert) && isAlertBelongsTo(alert, followUpTypes)) {
                return new RefillFollowUps(alert.name(), alert, Context.getInstance().getStringResource(org.ei.opensrp.R.string.str_follow_up));
            } else if (isOtherFPMethodAlert(alert) && isAlertBelongsTo(alert, refillTypes)) {
                return new RefillFollowUps(alert.name(), alert, Context.getInstance().getStringResource(org.ei.opensrp.R.string.str_refill));
            }
        }
        return null;
    }
    ...
    public String highPriorityReason() {
        return StringUtil.humanize(highPriorityReason);
    }
    ...
    public String familyPlanningMethodChangeDate() {
        return DateUtil.formatDate(family_planning_method_change_date, "dd/MM/YYYY");
    }
    ...
    public String iudPerson() {
        return StringUtil.humanizeAndDoUPPERCASE(iudPerson);
    }

    @Override
    public String iudPlace() {
        return StringUtil.humanizeAndDoUPPERCASE(iudPlace);
    }
    ...
    public String complicationDate() {
        return DateUtil.formatDate(complication_date, "dd/MM/YYYY");
    }

    @Override
    public String condomSideEffect() {
        return StringUtil.replaceAndHumanize(condomSideEffect, AllConstants.SPACE, AllConstants.COMMA_WITH_SPACE);
    }

    @Override
    public String iudSidEffect() {
        return StringUtil.replaceAndHumanize(iudSidEffect, AllConstants.SPACE, AllConstants.COMMA_WITH_SPACE);
    }

    @Override
    public String ocpSideEffect() {
        return StringUtil.replaceAndHumanize(ocpSideEffect, AllConstants.SPACE, AllConstants.COMMA_WITH_SPACE);
    }

    @Override
    public String sterilizationSideEffect() {
        return StringUtil.replaceAndHumanize(sterilizationSideEffect, AllConstants.SPACE, AllConstants.COMMA_WITH_SPACE);
    }

    @Override
    public String injectableSideEffect() {
        return StringUtil.replaceAndHumanize(injectableSideEffect, AllConstants.SPACE, AllConstants.COMMA_WITH_SPACE);
    }

    @Override
    public String otherSideEffect() {
        return StringUtil.replaceAndHumanize(otherSideEffect, AllConstants.SPACE, AllConstants.COMMA_WITH_SPACE);
    }
