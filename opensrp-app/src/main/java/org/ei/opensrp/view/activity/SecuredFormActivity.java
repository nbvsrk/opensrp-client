...
public abstract class SecuredFormActivity extends SecuredWebActivity {
    ...
    private void getIntentData() throws IOException {
        ...
        formName = intent.getStringExtra(AllConstants.FORM_NAME_PARAM);
        entityId = intent.getStringExtra(AllConstants.ENTITY_ID_PARAM);
        fieldOverrides = intent.getStringExtra(AllConstants.FIELD_OVERRIDES_PARAM);
    ...
    private void webViewInitialization() {
        ...
        webView.addJavascriptInterface(Context.getInstance().formDataRepository(), AllConstants.REPOSITORY);
        webView.addJavascriptInterface(Context.getInstance().ziggyFileLoader(), AllConstants.ZIGGY_FILE_LOADER);
        webView.addJavascriptInterface(Context.getInstance().formSubmissionRouter(), AllConstants.FORM_SUBMISSION_ROUTER);
        ...
        try {
            ...
            Log.logError(MessageFormat.format("Cannot encode field overrides: {0} due to : {1}", fieldOverrides, e));
        }
        webView.loadUrl(MessageFormat.format("file:///android_asset/www/enketo/template.html?{0}={1}&{2}={3}&{4}={5}&{6}={7}&touch=true",
                AllConstants.FORM_NAME_PARAM, formName,
                AllConstants.ENTITY_ID_PARAM, entityId,
                AllConstants.INSTANCE_ID_PARAM, randomUUID(),
                AllConstants.FIELD_OVERRIDES_PARAM, encodedFieldOverrides));
    }
