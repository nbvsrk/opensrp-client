        ...
        return cache.get(EC_CLIENTS_LIST, new CacheableData<String>() {
            ...
            public String fetch() {
                ...
                for (EligibleCouple ec : ecs) {
                    String photoPath = isBlank(ec.photoPath()) ? AllConstants.DEFAULT_WOMAN_IMAGE_PLACEHOLDER_PATH : ec.photoPath();
                    ECClient ecClient = new ECClient(ec.caseId(), ec.wifeName(), ec.husbandName(), ec.village(), IntegerUtil.tryParse(ec.ecNumber(), 0))
                            .withDateOfBirth(ec.getDetail(AllConstants.ECRegistrationFields.WOMAN_DOB))
                            .withFPMethod(ec.getDetail(AllConstants.ECRegistrationFields.CURRENT_FP_METHOD))
                            .withFamilyPlanningMethodChangeDate(ec.getDetail(AllConstants.ECRegistrationFields.FAMILY_PLANNING_METHOD_CHANGE_DATE))
                            .withIUDPlace(ec.getDetail(AllConstants.ECRegistrationFields.IUD_PLACE))
                            .withIUDPerson(ec.getDetail(AllConstants.ECRegistrationFields.IUD_PERSON))
                            .withNumberOfCondomsSupplied(ec.getDetail(AllConstants.ECRegistrationFields.NUMBER_OF_CONDOMS_SUPPLIED))
                            .withNumberOfCentchromanPillsDelivered(ec.getDetail(AllConstants.ECRegistrationFields.NUMBER_OF_CENTCHROMAN_PILLS_DELIVERED))
                            .withNumberOfOCPDelivered(ec.getDetail(AllConstants.ECRegistrationFields.NUMBER_OF_OCP_DELIVERED))
                            ...
                            .withNumberOfPregnancies(ec.getDetail(AllConstants.ECRegistrationFields.NUMBER_OF_PREGNANCIES))
                            .withParity(ec.getDetail(AllConstants.ECRegistrationFields.PARITY))
                            .withNumberOfLivingChildren(ec.getDetail(AllConstants.ECRegistrationFields.NUMBER_OF_LIVING_CHILDREN))
                            .withNumberOfStillBirths(ec.getDetail(AllConstants.ECRegistrationFields.NUMBER_OF_STILL_BIRTHS))
                            .withNumberOfAbortions(ec.getDetail(AllConstants.ECRegistrationFields.NUMBER_OF_ABORTIONS))
                            .withIsHighPriority(ec.isHighPriority())
                            .withPhotoPath(photoPath)
                            .withHighPriorityReason(ec.getDetail(AllConstants.ECRegistrationFields.HIGH_PRIORITY_REASON))
    ...
    private void updateStatusInformation(EligibleCouple eligibleCouple, ECClient ecClient) {
        ...
        if (mother == null && !eligibleCouple.hasFPMethod()) {
            ecClient.withStatus(EasyMap.create(STATUS_TYPE_FIELD, EC_STATUS)
                    .put(STATUS_DATE_FIELD, eligibleCouple.getDetail(AllConstants.ECRegistrationFields.REGISTRATION_DATE)).map());
            return;
        }

        if (mother == null && eligibleCouple.hasFPMethod()) {
            ecClient.withStatus(EasyMap.create(STATUS_TYPE_FIELD, FP_STATUS)
                    .put(STATUS_DATE_FIELD, eligibleCouple.getDetail(AllConstants.ECRegistrationFields.FAMILY_PLANNING_METHOD_CHANGE_DATE)).map());
            return;
        }
        ...
        if (mother != null && mother.isANC()) {
            ecClient.withStatus(EasyMap.create(STATUS_TYPE_FIELD, ANC_STATUS)
                    ...
                    .put(STATUS_EDD_FIELD, LocalDate.parse(mother.getDetail(AllConstants.ANCRegistrationFields.EDD), DateTimeFormat.forPattern(AllConstants.FORM_DATE_TIME_FORMAT)).toString()).map());
            return;
        }
        ...
        if (mother != null && mother.isPNC() && eligibleCouple.hasFPMethod()) {
            ecClient.withStatus(EasyMap.create(STATUS_TYPE_FIELD, PNC_FP_STATUS)
                    .put(STATUS_DATE_FIELD, mother.referenceDate())
                    .put(FP_METHOD_DATE_FIELD, eligibleCouple.getDetail(AllConstants.ECRegistrationFields.FAMILY_PLANNING_METHOD_CHANGE_DATE)).map());
        }
    }
}
