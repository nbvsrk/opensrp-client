...
public class ChildClient implements ChildSmartRegisterClient {
    ...
    static {
        categoriesToServiceTypeMap.put(CATEGORY_BCG, Arrays.asList(ChildServiceType.BCG));
        categoriesToServiceTypeMap.put(CATEGORY_MEASLES, Arrays.asList(ChildServiceType.MEASLES, ChildServiceType.MEASLESBOOSTER));
        categoriesToServiceTypeMap.put(CATEGORY_OPV, Arrays.asList(ChildServiceType.OPV_0, ChildServiceType.OPV_1, ChildServiceType.OPV_2, ChildServiceType.OPV_3));
        categoriesToServiceTypeMap.put(CATEGORY_OPVBOOSTER, Arrays.asList(ChildServiceType.OPV_BOOSTER));
        categoriesToServiceTypeMap.put(CATEGORY_DPT, Arrays.asList(ChildServiceType.DPTBOOSTER_1, ChildServiceType.DPTBOOSTER_2));
        categoriesToServiceTypeMap.put(CATEGORY_PENTAVALENT, Arrays.asList(ChildServiceType.PENTAVALENT_1, ChildServiceType.PENTAVALENT_2, ChildServiceType.PENTAVALENT_3));
        categoriesToServiceTypeMap.put(CATEGORY_HEPB, Arrays.asList(ChildServiceType.HEPB_0));
        categoriesToServiceTypeMap.put(CATEGORY_VITAMIN_A, Arrays.asList(ChildServiceType.VITAMIN_A));
        categoriesToServiceTypeMap.put(CATEGORY_CHILD_ILLNESS, Arrays.asList(ChildServiceType.ILLNESS_VISIT));
    }
    ...
    private class Treatments {
        public ServiceProvidedDTO provided = ServiceProvidedDTO.emptyService;
        public AlertDTO toProvide = AlertDTO.emptyAlert;
    }
    ...
    public String gender() {
        return StringUtil.humanize(gender);
    }
    ...
    @Override
    public boolean isSC() {
        return caste != null && caste.equalsIgnoreCase(AllConstants.ECRegistrationFields.SC_VALUE);
    }

    @Override
    public boolean isST() {
        return caste != null && caste.equalsIgnoreCase(AllConstants.ECRegistrationFields.ST_VALUE);
    }
    ...
    public boolean isBPL() {
        return economicStatus != null && economicStatus.equalsIgnoreCase(AllConstants.ECRegistrationFields.BPL_VALUE);
    }
    ...
    public String dateOfBirth() {
        return isBlank(dob) ? "" : DateUtil.formatDate(dob);
    }
    ...
    public ServiceProvidedDTO lastServiceProvided() {
        if (lastService == null) {
            lastService = serviceProvided().size() > 0
                    ...
                    : ServiceProvidedDTO.emptyService;
        }
        return lastService;
    }
    ...
    public ChildSickStatus sickStatus() {
        ...
        if (service == ServiceProvidedDTO.emptyService) {
            ...
            String date;
            if (data.containsKey(AllConstants.ChildIllnessFields.REPORT_CHILD_DISEASE)) {
                diseases = data.get(AllConstants.ChildIllnessFields.REPORT_CHILD_DISEASE);
                otherDiseases = data.get(AllConstants.ChildIllnessFields.REPORT_CHILD_DISEASE_OTHER);
                date = data.get(AllConstants.ChildIllnessFields.REPORT_CHILD_DISEASE_DATE);
            } else {
    ...
    public ChildClient withOutOfArea(boolean outOfArea) {
        this.locationStatus = outOfArea ? AllConstants.OUT_OF_AREA : AllConstants.IN_AREA;
        return this;
    }
    ...
    private boolean isServiceProvided(String category) {
        ...
        return serviceToTreatmentMap.get(category).provided != ServiceProvidedDTO.emptyService;
    }
