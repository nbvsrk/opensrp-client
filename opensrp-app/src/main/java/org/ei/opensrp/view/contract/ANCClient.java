...
public class ANCClient implements ANCSmartRegisterClient {
    ...
    public String eddForDisplay() {
        return DateUtil.formatDate(parse(edd).toLocalDate().toString(), "dd/MM/yy");
    }
    ...
    public String pastDueInDays() {
        return isBlank(edd) ? "0"
                : Integer.toString(Days.daysBetween(parse(edd).toLocalDate(), DateUtil.today()).getDays());
    }

    @Override
    public String weeksAfterLMP() {
        return isBlank(lmp) ? "0"
                : Integer.toString(Days.daysBetween(parse(lmp).toLocalDate(), DateUtil.today()).getDays() / 7);
    }
    ...
    @Override
    public String lmp() {
        return DateUtil.formatDate(lmp, "dd/MM/yy");
    }

    @Override
    public String riskFactors() {
        if (StringUtils.isNotEmpty(riskFactors)) {
            return StringUtil.replaceAndHumanize(riskFactors.trim(), AllConstants.SPACE, AllConstants.COMMA_WITH_SPACE);
        }
        return null;
    }
    ...
    private boolean isServiceProvided(String category) {
        ...
        return serviceToVisitsMap.get(category).provided != ServiceProvidedDTO.emptyService;
    }
    ...
    public String getHyperTension(ServiceProvidedDTO ancServiceProvided) {
        String systolic = ancServiceProvided.data().get(AllConstants.ANCVisitFields.BP_SYSTOLIC);
        String diastolic = ancServiceProvided.data().get(AllConstants.ANCVisitFields.BP_DIASTOLIC);
        if (StringUtils.isEmpty(systolic) && StringUtils.isEmpty(diastolic)) {
            return EMPTY;
        }
        return systolic + AllConstants.SLASH_STRING + diastolic;
    }
