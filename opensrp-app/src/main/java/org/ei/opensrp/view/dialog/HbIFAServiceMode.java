...
public class HbIFAServiceMode extends ServiceModeOption {
    ...
    @Override
    public String name() {
        return Context.getInstance().getStringResource(anc_service_mode_hb_ifa);
    }
    ...
    public void setupHbDetailsLayout(ANCSmartRegisterClient client,
        ...
        for (ServiceProvidedDTO serviceProvided : client.allServicesProvidedForAServiceType(HB_TEST.displayName())) {
            ...
            ViewGroup hbDetailsViewGroup = (ViewGroup) inflater.inflate(org.ei.opensrp.R.layout.smart_register_anc_hb_details_layout, null);
            ((TextView) hbDetailsViewGroup.findViewById(org.ei.opensrp.R.id.txt_hb_date)).setText(serviceProvided.shortDate());
            ((TextView) hbDetailsViewGroup.findViewById(org.ei.opensrp.R.id.txt_hb_level)).setText(hbLevel + Context.getInstance().getStringResource(anc_service_mode_hb_unit));
            hbDetailsViewGroup.findViewById(org.ei.opensrp.R.id.hb_level_indicator)
    ...
    public void setupIFADetailsLayout(ANCSmartRegisterClient client,
        ...
        for (ServiceProvidedDTO serviceProvided : client.allServicesProvidedForAServiceType(IFA.serviceName())) {
            ...
            ViewGroup ifaDetailsViewGroup = (ViewGroup) inflater.inflate(org.ei.opensrp.R.layout.smart_register_anc_ifa_details_layout, null);
            ((TextView) ifaDetailsViewGroup.findViewById(org.ei.opensrp.R.id.txt_ifa_date)).setText(serviceProvided.shortDate());
            ((TextView) ifaDetailsViewGroup.findViewById(org.ei.opensrp.R.id.txt_number_of_ifa_tablets))
                    .setText(numberOfIFATablets + Context.getInstance().getStringResource(anc_service_mode_ifa_tablets));
    ...
    private int getHbColor(String hbLevel) {
        ...
        if (hbValue < 7)
            return Context.getInstance().getColorResource(org.ei.opensrp.R.color.hb_level_dangerous);
        else if (hbValue >= 7 && hbValue < 11) {
            return Context.getInstance().getColorResource(org.ei.opensrp.R.color.hb_level_high);
        } else {
            return Context.getInstance().getColorResource(org.ei.opensrp.R.color.hb_level_normal);
        }
    }
    ...
    public void setupHbAlertLayout(ANCSmartRegisterClient client,
        ...
        if (hbAlert != AlertDTO.emptyAlert) {
    ...
    private void setServiceProvidedLayout(ANCSmartRegisterClient client, ServiceProvidedDTO serviceProvided, View serviceProvidedLayout,
        ...
        txtServiceType.setTextColor(Context.getInstance().getColorResource(org.ei.opensrp.R.color.text_black));

        txtServiceDate.setVisibility(VISIBLE);
        txtServiceDate.setText("On " + serviceProvided.shortDate());
        txtServiceDate.setTextColor(Context.getInstance().getColorResource(org.ei.opensrp.R.color.text_black));
    }
