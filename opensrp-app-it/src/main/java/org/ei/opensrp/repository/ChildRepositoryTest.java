...
public class ChildRepositoryTest extends AndroidTestCase {
    ...
    private static final Map<String, String> EXTRA_DETAILS = EasyMap.mapOf("some-key", "some-value");
    ...
    public void testShouldUpdateChild() throws Exception {
        ...
        Assert.assertEquals(child, repository.find("CASE A"));
    }
    ...
    public void testShouldFetchChildrenByTheirOwnCaseId() throws Exception {
        ...
        Assert.assertEquals(new Child("CASE A", "CASE X", "TC 1", "2012-06-09", "female", EXTRA_DETAILS), repository.find("CASE A"));
        Assert.assertEquals(new Child("CASE B", "CASE X", "TC 1", "2012-06-10", "female", EXTRA_DETAILS), repository.find("CASE B"));
        Assert.assertEquals(new Child("CASE C", "CASE X", "TC 1", "2012-06-10", "female", EXTRA_DETAILS).setIsClosed(true), repository.find("CASE C"));
    }
    ...
    public void testShouldMarkAsClosedWhenAChildIsClosed() throws Exception {
        ...
        Assert.assertEquals(firstChild.setIsClosed(true), repository.find(firstChild.caseId()));
        Assert.assertEquals(secondChild, repository.find(secondChild.caseId()));
    }
    ...
    public void testShouldUpdateMotherDetails() throws Exception {
        Map<String, String> details = EasyMap.mapOf("some-key", "some-value");
        Child child = new Child("CASE A", "CASE X", "TC 1", "2012-06-09", "female", details);
        repository.add(child);

        Map<String, String> newDetails = EasyMap.create("some-key", "some-new-value").put("some-other-key", "blah").map();
    ...
    public void testShouldUpdatePhotoPathCase() throws Exception {
        ...
        Assert.assertEquals("photo/path/to/child/x", repository.find("CASE X").photoPath());
        Assert.assertEquals(null, repository.find("CASE Y").photoPath());
    }
